...
function scoreSurfing(loc: LocationMetadata, w: WeatherSnapshot): SuitabilityResult {
  ...
  // Missing critical hazard data (native cliff — not a post-processor)
  if (w.dataQuality.missingCriticalHazards.length > 0) {
    score10 = Math.min(score10, 7.0); // 70/100 max — prevents GREAT without verified wave data
    reasons.push('Wave or swell data unavailable; surf conditions unverifiable.');
  }
...
function scoreSkiingSnowboarding(loc: LocationMetadata, w: WeatherSnapshot): SuitabilityResult {
  ...
  // Missing critical hazard data — fail closed (skiing/snowboarding are higher-risk)
  // Cannot score GREAT without verified snow depth, visibility, and gust data.
  if (w.dataQuality.missingCriticalHazards.length > 0) {
    maxScore = Math.min(maxScore, 6.0); // 60/100 max → OK at best; verify at resort
    reasons.push(
