  ...
  async function fetchResults(q: string) {
    ...
    try {
      const res = await fetch(
        ...
        { headers: { "User-Agent": `WeatherOrNot/1.0 (${UA_CONTACT})` } },
      );
  ...
  return (
    <div className={styles.root}>
      ...
      <div className={styles.inputWrap}>
        <HiMagnifyingGlass className={styles.icon} aria-hidden="true" />
        <input
          id={inputId}
          type="search"
          role="combobox"
          ...
          aria-activedescendant={
            expanded && results[activeIndex] ? `${listboxId}-${activeIndex}` : undefined
          }
          placeholder="Search a spot, trail, or beach"
          value={query}
          onChange={(e) => {
